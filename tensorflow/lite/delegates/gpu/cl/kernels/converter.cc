...
class TensorToBHWCBufferConverter : public OpenClConverterImpl {
 public:
  ...
  absl::Status Convert(const TensorObject& input_obj,
                       const TensorObject& output_obj) override {
    // TFLITE_LOG_PROD(TFLITE_LOG_INFO,"fsw In Convert 2...");
...
class BHWCBufferToTensorConverter : public OpenClConverterImpl {
 public:
  ...
  absl::Status Convert(const TensorObject& input_obj,
                       const TensorObject& output_obj) override {
    // TFLITE_LOG_PROD(TFLITE_LOG_INFO,"fsw In Convert 3...");
...
class TrivialCopier : public OpenClConverterImpl {
 public:
  ...
  absl::Status Convert(const TensorObject& input_obj,
                       const TensorObject& output_obj) override {
    // TFLITE_LOG_PROD(TFLITE_LOG_INFO,"fsw In Convert 4...");
...
class CpuCopier : public OpenClConverterImpl {
 public:
  ...
  absl::Status Convert(const TensorObject& input_obj,      //从inout_obj向output_obj copy 
                       const TensorObject& output_obj) override {   
    //author:fu                     
    // TFLITE_LOG_PROD(TFLITE_LOG_INFO,"fsw In Convert...");
    ...
    if (cpu_input) {
      ...
      if (buffer_output) {
        ...
        // cl_int error_code;
        // void* hostPtr = clEnqueueMapBuffer(queue_->queue(), buffer_output->memobj, CL_TRUE, CL_MAP_WRITE, 0, cpu_input->size_bytes, 0, NULL, NULL, &error_code);
        ...
        // TFLITE_LOG(INFO) << "fsw input_map time: " << d2.count() << "ms" << std::endl;
        ...
        // TFLITE_LOG(INFO) << "fsw input_map time: " << duration_mcs2.count() << "us" << std::endl;
        // memcpy(hostPtr, cpu_input->data, cpu_input->size_bytes);
        // error_code = clEnqueueUnmapMemObject(queue_->queue(), buffer_output->memobj, hostPtr, 0, NULL, NULL);
        // return absl::OkStatus();                            
      ...
      if (buffer_input) {
        ...
        // auto start = std::chrono::high_resolution_clock::now();
        // cl_int error_code;
        // void* hostPtr = clEnqueueMapBuffer(queue_->queue(), buffer_input->memobj, CL_TRUE, CL_MAP_READ, 0, cpu_output->size_bytes, 0, NULL, NULL, &error_code);
        // auto end = std::chrono::high_resolution_clock::now();
        // std::chrono::duration<double,std::ratio<1,1>> ds2 = end - start;
        // std::chrono::milliseconds d2 = std::chrono::duration_cast< std::chrono::milliseconds >( ds2 );
        // TFLITE_LOG(INFO) << "fsw map time: " << d2.count() << "ms" << std::endl;
        // std::chrono::duration<double,std::ratio<1,1000000>> duration_mcs2=std::chrono::duration_cast<std::chrono::duration<double,std::ratio<1,1000000>>> (end-start);  
        // TFLITE_LOG(INFO) << "fsw map time: " << duration_mcs2.count() << "us" << std::endl;
        // // memcpy(cpu_output->data, hostPtr, cpu_output->size_bytes);
        // error_code = clEnqueueUnmapMemObject(queue_->queue(), buffer_input->memobj, hostPtr, 0, NULL, NULL);
        // return absl::OkStatus();                            
      }
    }
    return absl::InternalError("Unexpected object");
