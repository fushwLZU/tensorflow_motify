...
TfLiteStatus InterpreterBuilder::operator()(
  ...
  for (int subgraph_index = 0; subgraph_index < subgraphs->size();
    ...
    //author:fu
    original_tensor = modified_subgraph->tensors_ptr();

    // for(int i=0;i<tensors->size();++i){
    //   TFLITE_LOG(INFO) << "tensor_id and name : " << i << " " << original_tensor[i].name;
    // }


    //author:fu
    // TFLITE_LOG(INFO) << "subgraph->tensors_size()=" << tensors->size() << std::endl;
    // TFLITE_LOG(INFO) << "modified_subgraph->tensors_size()=" << modified_subgraph->tensors_size() << std::endl;
    // TFLITE_LOG(INFO) << "modified_subgraph->inputs.size()=" << modified_subgraph->inputs().size() << std::endl;
    // for(auto x : modified_subgraph->inputs()){
    //   TFLITE_LOG(INFO) << x << " ";
    // }
    // TFLITE_LOG(INFO) << "modified_subgraph->outputs.size()=" << modified_subgraph->outputs().size() << std::endl;
    // for(auto x : modified_subgraph->outputs()){
    //   TFLITE_LOG(INFO) << x << " ";
    // }
