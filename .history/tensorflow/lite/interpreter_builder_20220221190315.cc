...
TfLiteStatus InterpreterBuilder::operator()(
  ...
  for (int subgraph_index = 0; subgraph_index < subgraphs->size();
    ...
    //author:fu
    TFLITE_LOG(INFO) << "subgraph->inputs.size()=" << modified_subgraph->inputs().size() << std::endl;
    TFLITE_LOG(INFO) << "subgraph->outputs.size()=" << modified_subgraph->outputs().size() << std::endl;
