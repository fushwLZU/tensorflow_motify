...
TfLiteStatus GraphPartitionHelper::Partition(
  ...
  //得到model中所有support的node存放在supported_nodes_中
...
TfLiteStatus GraphPartitionHelper::PrepareSupportedNodes(
  ...
  //author:fu
  // TFLITE_LOG(INFO) << "original_execution_plan_:" << std::endl;
  // for (int node_id : TfLiteIntArrayView(original_execution_plan_)){
  ...
  //
  auto it = gpu_supported_nodes.begin();
  gpu_supported_nodes.insert(it,0);
