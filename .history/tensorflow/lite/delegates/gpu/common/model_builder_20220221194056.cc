...
absl::Status BuildModelEnforceIO(
  ...
  absl::flat_hash_map<int, Value*> tensor_to_value;  //int是新的graph中tensor的idx，Value是对TfliteTensor的封装，value.tensor.ref指向这个tensor在subgraph.tensor_的index
  ...
  for(int ii=0;ii<graph->values().size();++ii){
    TFLITE_LOG(INFO) << tensor_to_value[ii] << " ";
  }
