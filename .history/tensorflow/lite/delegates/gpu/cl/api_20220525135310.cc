 ...
 public:
  ...
  std::vector<void*> map_output_buffer(){
    ...
    for(int i = 0; i < output_tensor_map_.size(); ++i){
      ...
      cl_int error_code;
      cl_event* map_out_event_ = &(map_out_event.event());
      void* hostPtr = tflite::gpu::cl::clEnqueueMapBuffer(queue_->queue(), buffer, CL_FALSE, CL_MAP_READ, 0, data_size, 0, NULL, map_out_event_, &error_code);
