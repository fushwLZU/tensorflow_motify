  ...
  absl::Status MaybeAllocateExternalObject(Environment* env) {
    ...
    switch (d.object_def.object_type) {
      ...
      case ObjectType::OPENCL_BUFFER: {
        ...
        // //author:fu
        // if(is_input_tensor_linked==false){
        //   input_tensor_map.push_back(OpenClBuffer{cl_memory_.memory()});
        // }
        // else{
        //   output_tensor_map.push_back(OpenClBuffer{cl_memory_.memory()});
        // }
...
class InferenceRunnerImpl : public CLInferenceRunner {
 public:
  ...
  absl::Status Run() override {
    ...
    //author:fu
    for(int i = 0; i < input_tensor_map.size(); ++i){
      cl_mem buffer = input_tensor_map[i].memobj;
      void* hostPtr = clEnqueueMapBuffer(queue_, buffer, CL_TRUE, CL_MAP_READ, 0, 
    }
