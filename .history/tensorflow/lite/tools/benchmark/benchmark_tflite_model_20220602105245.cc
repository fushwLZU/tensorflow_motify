...
std::string GetTensorName(std::unique_ptr<tflite::Interpreter> interpreter,
                          int tensor_index) {
...
std::vector<std::string> GetTensorNames(std::unique_ptr<tflite::Interpreter> interpreter,
...
void BenchmarkTfLiteModel::partitionModel(){
  for (int i = 0; i < interpreter_->execution_plan().size(); ++i) {
    ...
    std::vector<std::string> op_names = GetTensorNames(interpreter_, outputs);
